...
describe('set order status as transfer progress (e2e)', () => {
	...
	test('[PATCH] /orders/:orderId/set-status/transfer-progress', async () => {
		...
		// eslint-disable-next-line prettier/prettier
		const nextLocation = await distributionCenterFactory.makePrismaDistributionCenter()

		const response = await request(app.getHttpServer())
			.patch(`/orders/${orderId}/set-status/transfer-progress`)
			.set('Authorization', `Bearer ${accessToken}`)
			.send({
				nextLocationId: nextLocation.id.toString(),
